...
from ..kernels.wan_rope import B10WanRope
import os
import logging
...
@torch.profiler.record_function("rope_apply_dist")
...
def sp_dit_forward(
    ...
    if t.dim() == 1:
        with torch.amp.autocast('cuda', dtype=torch.float32):
            ...
            e = self.time_embedding(sinusoidal_embedding.unflatten(0, (bt, 1)).float())
        ...
        with torch.amp.autocast('cuda', dtype=torch.float32):
            ...
            t = t.flatten()
            sinusoidal_embedding = sinusoidal_embedding_1d(self.freq_dim, t)
            e = self.time_embedding(sinusoidal_embedding.unflatten(0, (bt, seq_len)).float())
            e0 = self.time_projection(e).unflatten(2, (6, self.dim))
            assert e.dtype == torch.float32 and e0.dtype == torch.float32
    ...
    if enable_b10_kernel() and grid_sizes.shape[0] == 1:
        assert seq_lens.max() % sp_size == 0, "seq_lens.max() must be divisible by sp_size"
