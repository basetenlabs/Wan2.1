# Adapted from https://github.com/NVIDIA/TensorRT-Model-Optimizer/blob/main/modelopt/torch/quantization/qtensor/mxfp4_tensor.py
import torch
...
class MXFP4QuantizeUtil:
    ...
    E2M1_bounds = torch.tensor([0.25, 0.75, 1.25, 1.75, 2.5, 3.5, 5], device="cuda")
    ...
    def quantize(cls, input: torch.Tensor, block_size: int | None) -> tuple:
        ...
        def cast_fp4(x):
            ...
            sign_bit = (2 - sign) // 2
            ord_ = torch.sum(
                (x.abs().unsqueeze(-1) - cls.E2M1_bounds) > 0, dim=-1
            )
        ...
        # input_q = input.to(torch.float32)
        e8m0_scale = (e8m0_scale + 127).to(torch.uint8) # (input[0], 1)
